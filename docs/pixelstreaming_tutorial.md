...
- In MATRIX UI, enable Pixel Streaming in the launch page and start the simulator.
- For script-based launch, pass `1` as the 4th argument to `run_sim.sh`; it adds `-PixelStreamingURL=ws://127.0.0.1:8888` to connect to the local signalling server.
- Ensure the signalling/web server from Step 2 is running.
...
- If ports are occupied, adjust the signalling/web ports in the server config or scripts, and update `-PixelStreamingURL` in `run_sim.sh` accordingly.
- If the page cannot connect, check:
  ...
  - Pixel Streaming is enabled on the sim side (UI toggle or script argument = 1).
