...
Follow these steps to create and load your custom scene:

1.  **Create a JSON File**  
    Create a new `.json` file or use an existing one. You can start by copying one of the examples below.

2.  **Define Your Scene**  
    Add elements like Cubes, Cylinders, and Pedestrians using the supported JSON format.

3.  **Deploy the Scene**  
    Rename your file to `scene.json` and place it in the `matrix/scene/` directory:
    ```bash
    cp your_custom_scene.json matrix/scene/scene.json
    ```
...
4.  **Launch Simulation**  
    ...
    ./sim_launcher
    ```
    In the launcher interface, select the **Custom Map** option to load your scene.
...
> **Note:** The keys in the root JSON object (e.g., `"Element1"`, `"Element2"`) act as unique identifiers for the parser and must be unique within the file.
